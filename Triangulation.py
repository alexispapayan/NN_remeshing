...
import pickle
from Neural_network import get_connectivity_network
...
def retriangulate(contour):
    ...
    chosen, cavities = triangulate2(procrustes, index, ordered_triangles)
    new_elements = ordered_triangles[chosen]

    while len(cavities) > 0:
        cavity = cavities.pop()
        sub = contour[cavity]

        if len(cavity) == 3:
            new_elements = np.append(new_elements, np.array([cavity]), axis=0)
        else:
            net = get_connectivity_network(sub.shape[0])

            sub_procrustes = apply_procrustes(sub)
...
def order_triangles(contour, table):
    n_triangles = len(contour)*(len(contour)-3)
    ordered_triangles = np.zeros((n_triangles, 3), dtype=np.int)
    score = np.zeros(n_triangles)
    o = 0
    for i in range(len(contour)):
        for j in range(2, len(contour)-1):
            ordered_triangles[o] = np.array([i, (i+1) % len(contour), (i+j) % len(contour)])
            score[o] = table[i,(i+j) % len(contour)]
            o += 1
    return ordered_triangles[np.argsort(-score)]
...
def apply_procrustes(polygon_points, ref_polygon=None, plot=False, full_output=False):
    ...
    if full_output:
        Rinv = R.T
        def inverse_transform(polygon):
            return np.dot(polygon - mu_ref_polygon, Rinv) / (norm_ss_ref_polygon_points * traceTA) * norm_ss_polygon_points + mu_polygon
        def tangent_transform(tangents):
            return np.dot(tangents, R)
        return polygon_transformed, inverse_transform, tangent_transform
...
def triangulate(polygon,ordered_quality_matrix,recursive=True):
    ...
    if recursive:
        ...
        for sub_polygon_indices in sub_polygon_list:
            if len(sub_polygon_indices)>=3:
                ...
                sub_elements,_,_ = triangulate(sub_polygon,sub_order_matrix,recursive=True)
    ...
    return set_elements, sub_element_list, triangulated
...
def triangulate2(contour, index, ordered_triangles):
    # Only for contours with no interior vertices
    if len(contour) == 3:
        return True, []

    elements = np.zeros(len(ordered_triangles), dtype=np.bool)
    cavities = []

    t = 0
    while not within(ordered_triangles[t], contour, index):
        t += 1
    elements[t] = True
    triangle = ordered_triangles[t]

    for r in range(1,3):
        edge = np.roll(triangle, r)[:2]
        i0 = np.nonzero(index == edge[0])[0][0]
        i1 = np.nonzero(index == edge[1])[0][0]
        if i1 != (i0+1) % len(contour):
            if i0 < i1:
                sub = np.arange(i0, i1+1)
            else:
                sub = np.concatenate([np.arange(i0, len(contour)), np.arange(0, i1+1)])
            sub_index = index[sub]
            valid = np.all(np.isin(ordered_triangles, sub_index), axis=1)
            if np.count_nonzero(valid) == 0:
                cavities.append(sub_index)
            # elif len(sub_index) == 3:
            #     elements[valid] = within(sub_index, contour, index)
            else:
                sub_contour = contour[sub]
                sub_ordered_triangles = ordered_triangles[valid]
                elements[valid], sub_cavities = triangulate2(sub_contour, sub_index, sub_ordered_triangles)
                cavities += sub_cavities

    return elements, cavities

def within(triangle, contour, index):
    within = True

    i0 = np.nonzero(index == triangle[0])[0][0]
    i1 = np.nonzero(index == triangle[1])[0][0]
    i2 = np.nonzero(index == triangle[2])[0][0]

    # check intersections
    m0 = contour[i2] - contour[i0]
    m1 = contour[i2] - contour[i1]
    for r in range(len(contour)):
        edge = np.array([r, (r+1) % len(contour)])
        if np.any(np.isin(edge, np.array([i0, i1, i2]))):
            continue
        l0 = contour[edge[1]] - contour[edge[0]]
        A = np.column_stack([l0, -m0])
        b = contour[i0] - contour[edge[0]]
        try:
        ...
        if np.all(np.logical_and(x > 0, x < 1)):
            within = False
            break
        A = np.column_stack([l0, -m1])
        b = contour[i1] - contour[edge[0]]
        try:
            x = np.linalg.solve(A, b)
        except:
            continue
    ...
    # check sign of total curvature
    if within:
        sign = np.zeros(2)
        if i1 < i2:
            sub_index = np.arange(i1, i2+1)
        else:
            sub_index = np.concatenate([np.arange(i1, len(contour)), np.arange(0, i2+1)])
        if len(sub_index) < 3:
            sub_index = np.array([i0, i1, i2])
        sub_contour = contour[sub_index]
        rolled = np.roll(sub_contour, 1, axis=0)
        turn0 = sub_contour[:,0]*rolled[:,1] - sub_contour[:,1]*rolled[:,0]
        sign[0] = np.sign(np.sum(turn0))

        if i2 < i0:
            sub_index = np.arange(i2, i0+1)
        else:
            sub_index = np.concatenate([np.arange(i2, len(contour)), np.arange(0, i0+1)])
        if len(sub_index) < 3:
            sub_index = np.array([i0, i1, i2])
        sub_contour = contour[sub_index]
        rolled = np.roll(sub_contour, 1, axis=0)
        turn1 = sub_contour[:,0]*rolled[:,1] - sub_contour[:,1]*rolled[:,0]
        sign[1] = np.sign(np.sum(turn1))

        within = np.all(sign >= 0) or np.all(sign <= 0)
